...
exports.createRide = async (req, res) => {
    try {
        ...
        const { carType, fromLocation, toLocation, availableSeats, price, date, time } = req.body;

        if (!carType || !fromLocation || !toLocation || !availableSeats || !price || !date || !time) {
        ...
        const existingRide = await Ride.findOne({ userId: user._id, date: formattedDate, time });
        if (existingRide) {
...
exports.updateRide = async (req, res) => {
    try {
        ...
        if (date || time) {
            ...
            if (conflictingRide) {
                return res.status(400).json({ message: "Conflict: You already have a ride scheduled at this date and time." });
            }
        }
